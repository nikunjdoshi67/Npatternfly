"use strict";
Object.defineProperty(exports, "__esModule", { value: true });
const tslib_1 = require("tslib");
const React = tslib_1.__importStar(require("react"));
const helpers_1 = require("../../helpers");
const title_1 = tslib_1.__importDefault(require("@patternfly/react-styles/css/components/Title/title"));
const bullseye_1 = tslib_1.__importDefault(require("@patternfly/react-styles/css/layouts/Bullseye/bullseye"));
const react_styles_1 = require("@patternfly/react-styles");
const Backdrop_1 = require("../Backdrop/Backdrop");
const ModalBoxBody_1 = require("./ModalBoxBody");
const ModalBoxHeader_1 = require("./ModalBoxHeader");
const ModalBoxCloseButton_1 = require("./ModalBoxCloseButton");
const ModalBox_1 = require("./ModalBox");
const ModalBoxFooter_1 = require("./ModalBoxFooter");
const ModalBoxDescription_1 = require("./ModalBoxDescription");
exports.ModalContent = (_a) => {
    var { children, className = '', isOpen = false, header = null, description = null, title, hideTitle = false, showClose = true, footer = null, actions = [], onClose = () => undefined, variant = 'default', width = -1, modalBoxAriaDescribedById = '', id = '', disableFocusTrap = false } = _a, props = tslib_1.__rest(_a, ["children", "className", "isOpen", "header", "description", "title", "hideTitle", "showClose", "footer", "actions", "onClose", "variant", "width", "modalBoxAriaDescribedById", "id", "disableFocusTrap"]);
    if (!isOpen) {
        return null;
    }
    const modalBoxHeader = header ? (React.createElement("div", { className: react_styles_1.css(title_1.default.title) }, header)) : (React.createElement(ModalBoxHeader_1.ModalBoxHeader, { hideTitle: hideTitle },
        " ",
        title,
        " "));
    const modalBoxFooter = footer ? (React.createElement(ModalBoxFooter_1.ModalBoxFooter, null, footer)) : (actions.length > 0 && React.createElement(ModalBoxFooter_1.ModalBoxFooter, null, actions));
    const boxStyle = width === -1 ? {} : { width };
    const modalBox = (React.createElement(ModalBox_1.ModalBox, { style: boxStyle, className: className, variant: variant, title: title, id: modalBoxAriaDescribedById || id },
        showClose && React.createElement(ModalBoxCloseButton_1.ModalBoxCloseButton, { onClose: onClose }),
        modalBoxHeader,
        description && React.createElement(ModalBoxDescription_1.ModalBoxDescription, { id: id }, description),
        React.createElement(ModalBoxBody_1.ModalBoxBody, Object.assign({}, props, (!description && { id })), children),
        modalBoxFooter));
    return (React.createElement(Backdrop_1.Backdrop, null,
        React.createElement(helpers_1.FocusTrap, { active: !disableFocusTrap, focusTrapOptions: { clickOutsideDeactivates: true }, className: react_styles_1.css(bullseye_1.default.bullseye) }, modalBox)));
};
//# sourceMappingURL=ModalContent.js.map